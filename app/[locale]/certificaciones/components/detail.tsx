...
export default function CertificacionDetail({
  ...
  const parsedSections = parseCertContent(cert.content);
  ...
  return (
    <div className="mb-24">
      ...
      <div className="relative max-w-5xl mx-auto px-8 -mt-32 z-20">
        <div className="bg-white/95 backdrop-blur rounded-2xl shadow-xl border border-black/5 p-8 md:p-10 space-y-10">
          {parsedSections.length > 0 ? (
            ...
            // 🔥 FALLBACK CRÍTICO (ESTO ES LO QUE FALTABA)
            <div
...
export function parseCertContent(html: string) {
  ...
  doc.querySelectorAll("h3").forEach((h3) => {
    ...
    while (next && next.tagName !== "H3") {
      if (next.tagName === "UL") {
        next.classList.remove("list-disc");
    ...
    if (title.toLowerCase().includes("tiempo"))
      icon = <Clock className="w-5 h-5 text-primary-color" />;

    if (title.toLowerCase().includes("propuesta"))
